...
@Component({
  selector: 'app-export-tiers-siigmp',
  templateUrl: './export-tiers-siigmp.component.html',
  styleUrls: ['./export-tiers-siigmp.component.css']
})
export class ExportTiersSiigmpComponent implements OnInit {
  ...
  valider () {
    ...
    this.loading = true;
    this.DonnerJson = {
      NIF : this.insertionTiers.get('NIF')?.value,
      STAT : this.insertionTiers.get('STAT')?.value,
...
get NIF () {
  return this.insertionTiers.get('NIF');
}
get STAT () {
  return this.insertionTiers.get('STAT');
}
